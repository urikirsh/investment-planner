...
def test_get_cached_ticker_lookup_in_exchange_returns_cached_result_without_refetch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls = {"count": 0}
    _install_default_lookup_service_with_url_payloads(
    ...
    assert get_cached_ticker_lookup_in_exchange(exchange=Exchange.NYSE, ticker="AAPL") is None
    loaded = lookup_ticker_in_exchange(exchange=Exchange.NYSE, ticker="AAPL")
    cached = get_cached_ticker_lookup_in_exchange(exchange=Exchange.NYSE, ticker="AAPL")
