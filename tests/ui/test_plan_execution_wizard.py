...
def test_show_current_plan_execution_step_limits_action_amount_to_two_decimals(
    monkeypatch: pytest.MonkeyPatch,
    make_plan_step: Callable[..., PlanStep],
    make_wizard_host: Callable[..., Any],
) -> None:
    host = make_wizard_host(steps=[make_plan_step(delta="12345.6789")])
    monkeypatch.setattr(
        wizard_mod,
        "resolve_cached_instrument_price_ils",
        lambda **_kwargs: Decimal("1234.56"),
    ...
    assert "Action: BUY 12,345.68 (ILS)" in host.screen_wizard.wiz_info.value
