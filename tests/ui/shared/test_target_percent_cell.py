...
def test_target_percent_cell_read_raw_text_returns_empty_when_metadata_is_missing() -> None:
    item = QTreeWidgetItem()
    item.setText(Col.TARGET_PCT.value, "25.0%")

    assert TargetPercentCell.read_raw_text(item) == ""


def test_target_percent_cell_read_raw_text_from_index_returns_empty_when_metadata_is_missing(qapp: object) -> None:
    _ = qapp
    tree = QTreeWidget()
    tree.setColumnCount(len(Col))
    item = QTreeWidgetItem(tree)
    ...
    assert TargetPercentCell.read_raw_text_from_index(index) == ""
